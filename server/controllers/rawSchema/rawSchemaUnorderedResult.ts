  ...
  aggregate = (batchId): Promise<any> => {
    return new Promise((resolv, reject) => {
      ...
      collectionToWork.count().then((data) => {
        quantity = data;
        console.log(`quantity_1: `, quantity);
        ...
        order.on('progress', (data) => {
          this.model.update(
            ...
            updateQuantity++;
            //console.log(`updateQuantity_: ${updateQuantity}`);
            //console.log(`quantity_: ${quantity}`);
  
            if (quantity >= 0 && updateQuantity >= quantity) {
              this.executeAggregation(batchId).then((result) => {
  ...
  executeAggregation = (batchId): Promise<any> => {
    return new Promise((resolv, reject) => {
      this.model.aggregate([
        {$project: {docRawSchema: 1, value: 1, my_version: 1}},
        {$group: {_id: '$docRawSchema', value: {$sum: '$value'}, my_version: {$last: '$my_version'}}},
