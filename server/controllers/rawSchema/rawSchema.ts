  ...
  aggregate = (batchId): Promise<any> => {
    return new Promise((resolv, reject) => {
      this.model.aggregate([
        {$project: {batchId: 1, docRawSchema: 1, value: 1, my_version: 1}},
        {$group: {_id: '$docRawSchema', value: {$sum: 1}, batchId: {$last: '$batchId'}, docRawSchema: {$last: '$docRawSchema'}, my_version: {$last: '$my_version'}}},
